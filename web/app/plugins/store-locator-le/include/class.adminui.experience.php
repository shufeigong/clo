...
if (! class_exists('SLPlus_AdminUI_UserExperience')) {
    ...
    class SLPlus_AdminUI_UserExperience extends SLPlus_BaseClass_Object {
        ...
        $this->settings->add_ItemToGroup( array(
            ...
            /**
             * Add Search Features Group
             * 
             * @param string $section_name
             */
            private function add_group_search_features( $section_name ) {
            ...
            /**
             * Add the Search Labels Group
             * 
             * @param string $section_name
             */
            private function add_group_search_labels( $section_name ) {
                    $group_name = __( 'Search Labels', 'store-locator-le' );
                    $this->settings->add_ItemToGroup( array(
                        'section'       => $section_name,
                        'group'         => $group_name,
                        'use_prefix'    => false,
                        'setting'       => 'sl_search_label',
                        'label'         => __('Address' , 'store-locator-le' ),
                        'description'   => __( 'Search form address label.', 'store-locator-le' ),
                        'value'         => get_option( 'sl_search_label' , __( 'Address / Zip', 'store-locator-le' ) ),   // TODO: serialize this variable into options or options_nojs                            
                    ) );
                    $this->settings->add_ItemToGroup( array(
                        'section'       => $section_name,
                        'group'         => $group_name,
                        'use_prefix'    => false,
                        'setting'       => 'sl_radius_label',
                        'label'         => __('Radius' , 'store-locator-le' ),
                        'description'   => __( 'Search form radius label.', 'store-locator-le' ),
                        'value'         => get_option( 'sl_radius_label' , __( 'Within', 'store-locator-le' ) ),   // TODO: serialize this variable into options or options_nojs                            
                    ) );                         
            }                
             ...
             * 
             * TODO: DEPRECATE when no longer in use (EM/ER/ES Legacay as well as PRO)
             */
    ...
    private function save_the_country() {
        ...
        if ( count( $this->update_info ) > 0 ) {
            ...
            function search_form_settings() {
                    ...
                    $this->add_group_search_features( $section_name );
                    $this->add_group_search_labels( $section_name );
