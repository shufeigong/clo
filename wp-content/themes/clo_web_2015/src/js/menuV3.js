function pageLoad(linkSplit, basicItems) {
  if (linkSplit != '') {
    ...
    //not basic item, is subitem in side menu
      ...
      $.get("/wp-json/wp/v2/pages/?slug=" + location.hash.substr(1), function(response) {
        })
     ...
     */
      ...
      $.get("/wp-json/wp/v2/pages/?slug=" + linkSplit[1], function(response) {
        ...
        .done(function(response) {
          var content = response[0].content.rendered;
          if ($(".entry-content2").length < 1) {
            $(".entry-content").html(content).slideDown();
          }
          else {
            $(".entry-content2").slideDown();
          }
...
function change(objectId, itemId, thisid) {
  $.get("/wp-json/wp/v2/pages/" + objectId, function(response) {
    })
    .done(function(response) {
...
$(document).ready(function() {
  ...
  $("#skiplinks").children("a").click(function(e) {
    ...
    e.preventDefault();
    if ($(".menu-main-menu-container").length > 0) {
      location.href = "/site-map";
    } else {
      location.href = "/fr-site-map";
    }
...
function timeline() {
  ...
  $.artwl_bind({
    showbtnid: "btn_show",
    title: "Community Living Ontario: Milestones",
...
function gridClick(thisGrid) {
  ...
  $(thisGrid).parents("table").find("td").each(function() {
    $(this).removeClass("tdSelected")
  });
  $(thisGrid).parent().addClass("tdSelected");
  $(thisGrid).parents(".albumGrid").nextAll(".imgShowBox").children(".imgShow").children().attr({
    "src": newSrc,
    "alt": newAlt
  });
...
function leftArrowClick(thisArrow) {
  ...
  if (currentIndex > 0) {
    ...
    $("#album_message").find("table").find("td").each(function() {
      $(this).removeClass("tdSelected")
    });
